    ...
    init() {
        ...
        browser.webRequest.onAuthRequired.addListener(
            async function (details) {
                if (details.isProxy) {
                    const result = await proxifiedContainers.retrieve(details.cookieStoreId);
                    return {authCredentials: {username: result.proxy.username, password: result.proxy.password}};
                }
            },
            {urls: ["<all_urls>"]},
            ["blocking"]
